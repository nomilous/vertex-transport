...
class VertexSocket extends EventEmitter {
  ...
  _onMessage(message, detail) {
    ...
    if (detail.binary) {
      ...
      if (decoded.meta.ack || decoded.meta.nak) {
        if (!this._waiting[decoded.meta.seq]) {
          try {
            this.emit('error', new errors.VertexSocketReplyError('Stray ack or nak', decoded));
          }
          catch (e) {
            /* in case of no error handler */
          }
    ...
    let response = {meta: decoded.meta}; // allows for insertion of 3rd part meta by on'data' handler
  ...
  _format(decoded) {
    ...
    if (decoded.data) {
      Object.keys(decoded.data).forEach(key => {
        if (!decoded.data[key]._error) return;
        decoded.data[key] = this._toError(decoded.data[key]);
      });
    }
    return decoded;
  }
