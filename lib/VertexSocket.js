...
class VertexSocket extends EventEmitter {
  ...
  write(data, timeout) {
    ...
    return new Promise((resolve, reject) => {
      ...
      header[0] = HEADER_1;
      header[1] = HEADER_2;
      header.writeUInt32LE(length, OFFSET_LENGTH);
      header.writeUInt32LE(sequence, OFFSET_SEQUENCE);
      header.writeDoubleLE(ts, OFFSET_TIMESTAMP);
  ...
  _onData(part) {
    ...
    while (this.buffer.length >= HEADER_LENGTH) {
      ...
      let length = this.buffer.readUInt32LE(OFFSET_LENGTH);
      let sequence = this.buffer.readUInt32LE(OFFSET_SEQUENCE);
      let ts = this.buffer.readDoubleLE(OFFSET_TIMESTAMP);
      let meta = {
        seq: sequence,
        ts: ts
      };
