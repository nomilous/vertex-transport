...
class VertexSocket extends EventEmitter {
  ...
  _format(meta, data) {
    ...
    if (typeof data !== 'undefined') formatted.data = data;

    if (formatted.meta.nak) {
      let error = this._toError(formatted.data);
      error.meta = formatted.meta;
      return error;
    }

    if (formatted.data) {
      Object.keys(formatted.data).forEach(key => {
        if (!formatted.data[key]._error) return;
        formatted.data[key] = this._toError(formatted.data[key]);
      });
    }
    return formatted;
  }
